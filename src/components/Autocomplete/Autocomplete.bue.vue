<template>
  ...
  >
    ...
    <div
      class="octo-autocomplete__dropdown-menu"
      :class="{ 'is-opened-top': !isListInViewportVertically }"
      v-show="isActive && (data.length > 0 || hasEmptySlot || hasHeaderSlot)"
      ref="refDropdown"
    >
      <div class="octo-autocomplete__dropdown-content" v-show="isActive">
        <div v-if="hasHeaderSlot" class="octo-autocomplete__dropdown-item">
          <slot name="header" />
        </div>
        <button
          type="button"
          ...
          :key="index"
          class="octo-autocomplete__dropdown-item"
          :class="{ 'is-hovered': option === hovered }"
          @click="setSelected(option)"
        >
          <slot v-if="hasDefaultSlot" :option="option" :index="index" />
          <span v-else>{{ getValue(option, true) }}</span>
        </button>
        <div
          v-if="data.length === 0 && hasEmptySlot"
          class="octo-autocomplete__dropdown-item is-disabled"
        >
          <slot name="empty" />
        </div>
        <div v-if="hasFooterSlot" class="octo-autocomplete__dropdown-item">
          <slot name="footer" />
        </div>
      </div>
    </div>
