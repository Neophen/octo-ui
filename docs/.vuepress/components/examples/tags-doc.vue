<template>
  <div class="container">
    <div class="control-box mb-4">
      <o-h size="3" class="mb-4">Simple tags</o-h>
      <o-field label="Add some tags" class="mb-4">
        <o-tags v-model="tags" ellipsis placeholder="Add a tag"></o-tags>
      </o-field>
      <p class="content">
        <b>Tags:</b>
        {{ tags }}
...
export default {
  setup() {
    ...
    const getFilteredTags = computed(() => text => {
      ...
      state.filteredTags = data.filter(option => {
        return (
    ...
    return { ...toRefs(state), getFilteredTags };
  }
};
</script>